...
| FIX-02 | high | Fixture suite: 24 names, deterministic traces/hashes, bounds, tie/zero/gem/undo coverage | python | `tests/test_fixture_plans.py` (11 cases) | pr | CI log |
| FIX-03 | high | Hosted fixture seed twice is idempotent; verify ranks/retry stability | backend | `dev/seed_hosted_fixtures.py --local/--hosted`; `dev/hosted_fixture_e2e.py` | nightly/release | `hosted-fixture-e2e.json` |
